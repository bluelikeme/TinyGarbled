...
public:
	...
	void Mask(uint32_t progress, uint32_t processedOTs, CBitVector* values,
			CBitVector* snd_buf, BYTE protocol) {
		uint32_t nsndvals = 2;

		if (protocol == G_OT) {
			snd_buf[0].XORBytes(
					values[0].GetArr() + ceil_divide(progress * m_nBitLength, 8),
					0, ceil_divide(processedOTs * m_nBitLength, 8));
			snd_buf[1].XORBytes(
					values[1].GetArr() + ceil_divide(progress * m_nBitLength, 8),
					0, ceil_divide(processedOTs * m_nBitLength, 8));
		} else if (protocol == C_OT) {
			values[0].SetBytes(snd_buf[0].GetArr(),
					ceil_divide(progress * m_nBitLength, 8),
					ceil_divide(processedOTs * m_nBitLength, 8)); //.SetBits(hash_buf, i*m_nBitLength, m_nBitLength);
			...
			values[1].SetBytes(snd_buf[1].GetArr(),
					ceil_divide(progress * m_nBitLength, 8),
					ceil_divide(processedOTs * m_nBitLength, 8));
		}
	}
	...
	void UnMask(uint32_t progress, uint32_t processedOTs, CBitVector& choices,
			CBitVector& output, CBitVector& rcv_buf, CBitVector& tmpmask,
			BYTE protocol) {
		...
		if (protocol == G_OT) {
			for (uint32_t u, i = progress, offset = processedOTs * bytelen, l =
					0; i < lim; i++, gprogress += bytelen, l += bytelen) {
				//TODO make this working for single bits
				u = (uint32_t) choices.GetBitNoMask(i);
				output.SetXOR(rcv_buf.GetArr() + (u * offset) + l,
						tmpmask.GetArr() + gprogress, gprogress, bytelen);
			}

		} else if (protocol == C_OT) {
			int gprogress = progress * bytelen;
			output.Copy(tmpmask.GetArr() + gprogress, gprogress,
					bytelen * processedOTs);
			for (int i = progress, l = 0; i < lim;
					i++, l += bytelen, gprogress += bytelen) {
	...
	void expandMask(CBitVector& out, BYTE* sbp, uint32_t offset,
			uint32_t processedOTs, uint32_t bitlength, crypto* crypt) {

		if (bitlength <= AES_KEY_BITS) {
			for (uint32_t i = 0; i < processedOTs; i++, sbp += AES_KEY_BYTES) {
				out.SetBits(sbp, (uint64_t) (offset + i) * bitlength,
						(uint64_t) bitlength);
			}
		} else {
